...
type RestOf<T extends IAuthor | NewAuthor> = Omit<T, 'createDate' | 'resetDate' | 'lastModifiedDate'> & {
...
export class AuthorService {
  ...
  protected convertDateFromClient<T extends IAuthor | NewAuthor | PartialUpdateAuthor>(author: T): RestOf<T> {
    return {
      ...author,
      createDate: author.createDate?.toJSON() ?? null,
      resetDate: author.resetDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restAuthor: RestAuthor): IAuthor {
    return {
      ...restAuthor,
      createDate: restAuthor.createDate ? dayjs(restAuthor.createDate) : undefined,
      resetDate: restAuthor.resetDate ? dayjs(restAuthor.resetDate) : undefined,
