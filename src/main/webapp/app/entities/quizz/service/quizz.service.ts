...
type RestOf<T extends IQuizz | NewQuizz> = Omit<T, 'examDate'> & {
...
export class QuizzService {
  ...
  protected convertDateFromClient<T extends IQuizz | NewQuizz | PartialUpdateQuizz>(quizz: T): RestOf<T> {
    return {
      ...quizz,
      examDate: quizz.examDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restQuizz: RestQuizz): IQuizz {
    return {
      ...restQuizz,
      examDate: restQuizz.examDate ? dayjs(restQuizz.examDate) : undefined,
    };
  }
