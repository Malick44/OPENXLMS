...
type RestOf<T extends IEnrollment | NewEnrollment> = Omit<T, 'enrolledDate' | 'completedDate'> & {
...
export class EnrollmentService {
  ...
  getEnrollmentIdentifier(enrollment: Pick<IEnrollment, 'id'>): string {
  ...
  protected convertDateFromClient<T extends IEnrollment | NewEnrollment | PartialUpdateEnrollment>(enrollment: T): RestOf<T> {
    return {
      ...enrollment,
      enrolledDate: enrollment.enrolledDate?.format(DATE_FORMAT) ?? null,
      completedDate: enrollment.completedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEnrollment: RestEnrollment): IEnrollment {
    return {
      ...restEnrollment,
      enrolledDate: restEnrollment.enrolledDate ? dayjs(restEnrollment.enrolledDate) : undefined,
      completedDate: restEnrollment.completedDate ? dayjs(restEnrollment.completedDate) : undefined,
    };
  }
