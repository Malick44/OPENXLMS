...
type RestOf<T extends IRating | NewRating> = Omit<T, 'timestamp'> & {
...
export class RatingService {
  ...
  protected convertDateFromClient<T extends IRating | NewRating | PartialUpdateRating>(rating: T): RestOf<T> {
    return {
      ...rating,
      timestamp: rating.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restRating: RestRating): IRating {
    return {
      ...restRating,
      timestamp: restRating.timestamp ? dayjs(restRating.timestamp) : undefined,
    };
  }
